...
class Authenticator(interface.InterfacesWithSafe):
    def __init__(self):
        self.ffi=self.ffi_auth      # Uses Authlib exclusively
        self.queue = queue.Queue()  # Each object has it's own queue for ffi calls
        self.rebind_ffi()

    def rebind_ffi(self, timeout=5):
        _bind_ffi_calls(self, timeout=timeout)

    ## Now, public methods here


#############################
#  Binding the FFI library methods used by Authenticator public methods
#############################
#
action_timeout_overrides ={'_auth_init_logging':6,
                           '_login' : 8}

def _bind_ffi_calls(self, timeout=5):
